...
$btnEditPagAdmin = filter_input(INPUT_POST, 'btnEditPagAdmin', FILTER_SANITIZE_STRING);
if ($btnEditPagAdmin) {
    ...
    if (in_array('', $dados)) {
        ...
        $result_usuario = "SELECT id FROM pag_admin WHERE name_pag='" . $dados['name_pag'] . "' AND id<>'" . $dados['id'] . "'";
        ...
        $result_usuario = "SELECT id FROM pag_admin WHERE caminho='" . $dados['caminho'] . "' AND id<>'" . $dados['id'] . "'";
        $resultado_usuario = mysqli_query($conectar, $result_usuario);
        if (($resultado_usuario) AND ($resultado_usuario->num_rows > 0)) {
            $erro = true;
    ...
    //var_dump($dados);
    if (!$erro) {
        $result_usuario = "UPDATE pag_admin set name_pag='" . $dados['name_pag'] . "', caminho='" . $dados['caminho'] . "', usuarioalt='$usuariologin', alterado= NOW() WHERE id='" . $dados['id'] . "'";
        $resultado_usuario = mysqli_query($conectar, $result_usuario);
